...
def canonical_household(monkeypatch, orm_session_factory):
    ...
    household = service.create_household("us", INPUTS, "Original", spm=NATIONAL)
...
def test_historical_zero_padded_reference_prevents_household_changes(
    canonical_household, orm_session_factory
):
    ...
    with pytest.raises(SPMValidationError) as error:
        service.update_household(
            "us", household.id, INPUTS, "Original", spm={"geography_kind": "county"}
        )
...
def test_legacy_label_edit_does_not_reinterpret_inputs_when_certificate_unavailable(
    ...
    service = HouseholdService(orm_session_factory)
    service.update_household("us", household_id, deepcopy(INPUTS), "New label")
