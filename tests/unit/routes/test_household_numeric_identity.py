"""Only strict US numeric simulation references make households immutable."""
...
def test_unrelated_simulation_population_id_does_not_freeze_household(
    ...
    assert control.status_code == 200, control.json
    ...
    assert stored["household_json"] == changed
    assert stored["spm"] == original["spm"] == NATIONAL
    assert stored["household_hash"] != original["household_hash"]
    assert stored["label"] == "After simulation"
...
def test_real_numeric_link_protects_inputs_but_allows_label_changes(
    ...
    stored = client.get(url).json["result"]
    assert stored == {**original, "label": "Renamed"}
    saved = client.get(f"/us/simulation/{simulation_id}").json["result"]
