...
def test_user_policy_nullable_identity_list_and_update_are_orm_managed():
    ...
    with uow.transaction() as daos:
        ...
        assert daos.user_policies.find_unique(**values)["id"] == user_policy_id
        assert daos.user_policies.update(user_policy_id, {"number_of_provisions": 4})
    with uow.read() as daos:
        rows = daos.user_policies.list_for_user("us", "auth0|one")
...
def test_economy_and_user_policy_daos_cover_lookup_edge_cases():
    ...
    with uow.transaction() as daos:
        economy_id = daos.economies.create(
            policy_id=2,
            country_id="us",
            region="us",
            time_period="2026",
        ...
        user_policy_id = daos.user_policies.create(**user_policy_values)
        assert daos.user_policies.update(999, {}) is False

    with uow.read() as daos:
        assert daos.economies.get(economy_id)["economy_json"] == {"result": 1}
        assert daos.economies.get(999) is None
        assert daos.user_policies.get(user_policy_id)["country_id"] == "us"
        assert daos.user_policies.get(999) is None
