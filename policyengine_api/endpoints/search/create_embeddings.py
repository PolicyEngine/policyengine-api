...
for country_id in COUNTRIES:
    metadata = requests.get(f"https://api.policyengine.org/{country_id}/metadata").json().get("result")
    variable_metadata = pd.DataFrame(dict(
    ...
    parameter_metadata = pd.DataFrame(dict(
    ...
    parameter_metadata = parameter_metadata[
        np.array([metadata["parameters"][name]["type"] == "parameter" for name in metadata["parameters"]])
    ]
...
for i, row in tqdm(metadata_df.iterrows(), total=len(metadata_df), desc="Creating embeddings"):
    try:
...
embeddings = np.array(metadata_df["embedding"].values.tolist()).reshape(-1, embedding_dimensions)
